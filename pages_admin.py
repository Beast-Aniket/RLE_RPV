...
def dataframe_to_excel_bytes(df: pd.DataFrame) -> bytes:
    ...
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="AdminReport")
    buffer.seek(0)
    return buffer.read()
...
def _apply_filters(df, date_from, date_to, session_name, exam_label, prn, seat, status_filter):
    out = df.copy()
    out["created_day"] = out["created_at"].str[:10]
    out = out[(out["created_day"] >= str(date_from)) & (out["created_day"] <= str(date_to))]
    if session_name != "All":
...
def render_admin_page(conn):
    ...
    m1.metric("Pending", int((df["status"] == "SUBMITTED_BY_CLERK").sum()))
    m2.metric("Approved", int((df["status"] == "ADMIN_APPROVED").sum()))
    m3.metric("Rejected", int((df["status"] == "ADMIN_REJECTED").sum()))
    m4.metric("Suggested", int((df["status"] == "ADMIN_SUGGESTED_EDIT").sum()))

    r1, r2 = st.columns(2)
    if r1.button("Today's Report"):
        st.session_state["admin_from"] = today
        st.session_state["admin_to"] = today
    if r2.button("Yesterday's Report"):
        st.session_state["admin_from"] = yesterday
        st.session_state["admin_to"] = yesterday

    f1, f2, f3, f4 = st.columns(4)
    date_from = f1.date_input("From", value=datetime.strptime(st.session_state.get("admin_from", today), "%Y-%m-%d").date())
    date_to = f2.date_input("To", value=datetime.strptime(st.session_state.get("admin_to", today), "%Y-%m-%d").date())
    ...
    prn = g1.text_input("Search PRN")
    seat = g2.text_input("Search Seat No")
    status_filter = g3.selectbox("Request Status", ["All", "SUBMITTED_BY_CLERK", "ADMIN_APPROVED", "ADMIN_REJECTED", "ADMIN_SUGGESTED_EDIT"])

    filtered = _apply_filters(df, date_from, date_to, session_name, exam_label, prn, seat, status_filter)
    st.dataframe(
        filtered[["id", "session_name", "exam_label", "name", "prn", "seat_no", "request_type", "status", "created_at", "admin_comment"]],
        use_container_width=True,
    )

    st.download_button("Download Filtered Requests (CSV)", filtered.to_csv(index=False).encode("utf-8"), file_name="admin_filtered_requests.csv", mime="text/csv")
    st.download_button("Download Filtered Requests (Excel)", dataframe_to_excel_bytes(filtered), file_name="admin_filtered_requests.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    if filtered.empty:
        return

    details = {f"#{r['id']} | {r['name']} | {r['status']}": r for _, r in filtered.iterrows()}
    chosen = details[st.selectbox("Select Request", list(details.keys()))]
    ...
    if a1.button("Approve"):
        ...
        conn.execute(
            "INSERT INTO final_actions(request_id,final_state,final_comment,updated_by,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(request_id) DO UPDATE SET final_state=excluded.final_state, final_comment=excluded.final_comment, updated_by=excluded.updated_by, updated_at=excluded.updated_at",
    ...
    if pdf_rows:
        ...
        s_f = st.selectbox("PDF Session", ["All"] + sorted(p_df["session_name"].unique().tolist()))
        e_f = st.selectbox("PDF Exam", ["All"] + sorted(p_df["exam_label"].unique().tolist()))
        d_f = st.text_input("PDF Day (YYYY-MM-DD)")
        out = p_df.copy()
        if s_f != "All":
            out = out[out["session_name"] == s_f]
        if e_f != "All":
            out = out[out["exam_label"] == e_f]
        if d_f:
            out = out[out["created_day"] == d_f]
        files = out["pdf_path"].tolist()
        if files:
            st.download_button("Bulk Download PDFs (ZIP)", download_zip_from_paths(files), file_name="admin_filtered_pdfs.zip", mime="application/zip")
