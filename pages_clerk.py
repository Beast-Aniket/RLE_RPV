...
def render_clerk_page(conn):
    ...
    if q:
        ...
        if not st_row:
            ...
            st.success(f"{st_row['name']} | PRN: {st_row['prn']} | Exam: {selected['exam_name']} ({selected['program_code']})")
            st.caption(f"Current status: {st_row['result_status']} | Current remark: {st_row['remark']} | Current CGPI: {st_row['cgpi']}")
            sem = {}
            cols = st.columns(6)
            for i in range(1, 7):
                sem[f"sem{i}"] = cols[i - 1].text_input(
                    f"Sem{i}", value="" if st_row[f"sem{i}"] is None else str(st_row[f"sem{i}"])
                )
