...
def render_final_page(conn):
    ...
    out = df.copy()
    if session_filter != "All": out = out[out["session_name"] == session_filter]
    if exam_filter != "All": out = out[out["exam_label"] == exam_filter]
    if day_filter: out = out[out["request_day"] == day_filter]

    st.dataframe(out[["request_id", "session_name", "exam_label", "name", "prn", "seat_no", "request_day", "final_state"]], use_container_width=True)

    st.download_button("Download Filtered Excel", out.to_csv(index=False).encode("utf-8"), file_name="final_filtered_entries.csv", mime="text/csv")

    ids = out["request_id"].tolist()
    selected = st.multiselect("Select request IDs", ids)
    a1, a2 = st.columns(2)
    ...
    if one.get("pdf_path"):
        ...
        with open(one["pdf_path"], "rb") as f:
            st.download_button("Download first filtered PDF", f.read(), file_name=one["pdf_path"].split("/")[-1], mime="application/pdf")
